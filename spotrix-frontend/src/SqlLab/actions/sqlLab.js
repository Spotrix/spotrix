...
export function estimateQueryCost(query) {
  ...
  const endpoint =
    schema === null
      ? `/spotrix/estimate_query_cost/${dbId}/`
      : `/spotrix/estimate_query_cost/${dbId}/${schema}/`;
  return dispatch =>
    Promise.all([
...
export function fetchQueryResults(query, displayLimit) {
  return function (dispatch) {
    ...
    return SupersetClient.get({
      endpoint: `/spotrix/results/${query.resultsKey}/?rows=${displayLimit}`,
      parseMethod: 'text',
    })
      .then(({ text = '{}' }) => {
...
export function validateQuery(query) {
  return function (dispatch) {
    ...
    return SupersetClient.post({
      endpoint: `/spotrix/validate_sql_json/${window.location.search}`,
      postPayload,
      stringify: false,
    })
...
function getTableExtendedMetadata(table, query, dispatch) {
  return SupersetClient.get({
    endpoint: encodeURI(
      `/spotrix/extra_table_metadata/${query.dbId}/` +
...
export function popDatasourceQuery(datasourceKey, sql) {
  return function (dispatch) {
    return SupersetClient.get({
      endpoint: `/spotrix/fetch_datasource_metadata?datasourceKey=${datasourceKey}`,
    })
      .then(({ json }) =>
        dispatch(
          addQueryEditor({
...
export function createCtasDatasource(vizOptions) {
  return dispatch => {
    dispatch(createDatasourceStarted());
    return SupersetClient.post({
      endpoint: '/spotrix/get_or_create_table/',
      postPayload: { data: vizOptions },
    })
      .then(({ json }) => {
        dispatch(createDatasourceSuccess(json));
