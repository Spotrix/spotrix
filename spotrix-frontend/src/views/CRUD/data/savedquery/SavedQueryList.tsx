...
function SavedQueryList({
  ...
  const openNewQuery = () => {
    window.open(`${window.location.origin}/spotrix/sqllab?new=true`);
  };
  ...
  const openInSqlLab = (id: number) => {
    window.open(`${window.location.origin}/spotrix/sqllab?savedQueryId=${id}`);
  };

  const copyQueryLink = useCallback(
    (id: number) => {
      copyTextToClipboard(
        `${window.location.origin}/spotrix/sqllab?savedQueryId=${id}`,
      )
        .then(() => {
