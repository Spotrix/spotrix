  ...
  const renderMenu = (query: Query) => (
    <Menu>
      {canEdit && (
        <Menu.Item
          onClick={() => {
            window.location.href = `/spotrix/sqllab?savedQueryId=${query.id}`;
          }}
        >
          {t('Edit')}
        </Menu.Item>
        ...
        buttons={[
          {
            ...
            onClick: () => {
              window.location.href = '/spotrix/sqllab?new=true';
            },
          },
          {
            name: 'View All »',
      ...
      {queries.length > 0 ? (
        <CardContainer showThumbnails={showThumbnails}>
          {queries.map(q => (
            <CardStyles
              onClick={() => {
                window.location.href = `/spotrix/sqllab?savedQueryId=${q.id}`;
              }}
              key={q.id}
            >
              <ListViewCard
                imgURL=""
                url={`/spotrix/sqllab?savedQueryId=${q.id}`}
                title={q.label}
