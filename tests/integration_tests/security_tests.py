...
from spotrix import app, appbuilder, db, security_manager, viz, ConnectorRegistry
from spotrix.connectors.druid.models import DruidCluster, DruidDatasource
from spotrix.connectors.sqla.models import RowLevelSecurityFilter, SqlaTable
...
class TestRolePermission(SupersetTestCase):
    ...
    def test_gamma_user_schema_access_to_dashboards(self):
        ...
        self.assertIn("/spotrix/dashboard/world_health/", data)
        self.assertNotIn("/spotrix/dashboard/births/", data)
...
class TestSecurityManager(SupersetTestCase):
    ...
    @patch("superset.security.SpotrixSecurityManager.raise_for_access")
    def test_can_access_datasource(self, mock_raise_for_access):
        ...
        mock_raise_for_access.side_effect = SpotrixSecurityException(
            SpotrixError(
                "dummy",
                SpotrixErrorType.DATASOURCE_SECURITY_ACCESS_ERROR,
                ErrorLevel.ERROR,
            )
        )
    ...
    def test_can_access_table(self, mock_raise_for_access):
        ...
        mock_raise_for_access.side_effect = SpotrixSecurityException(
            SpotrixError(
                "dummy",
                SpotrixErrorType.TABLE_SECURITY_ACCESS_ERROR,
                ErrorLevel.ERROR,
            )
        )
    ...
    @patch("superset.security.SpotrixSecurityManager.can_access")
    @patch("superset.security.SpotrixSecurityManager.can_access_schema")
    def test_raise_for_access_datasource(self, mock_can_access_schema, mock_can_access):
        ...
        with self.assertRaises(SpotrixSecurityException):
            security_manager.raise_for_access(datasource=datasource)
...
class TestDatasources(SupersetTestCase):
    @patch("superset.security.manager.g")
    @patch("superset.security.SpotrixSecurityManager.can_access_database")
    @patch("superset.security.SpotrixSecurityManager.get_session")
