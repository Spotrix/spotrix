...
from spotrix.tasks.alerts.validator import (
    AlertValidatorType,
    check_validator,
    not_null_validator,
    operator_validator,
)
from spotrix.tasks.schedules import (
    AlertState,
    deliver_alert,
    evaluate_alert,
...
def test_check_validator_error(description, validator_type, config):
    logger.info(description)
    with pytest.raises(SpotrixException):
...
def test_deliver_alert_screenshot(
    ...
    url_mock.side_effect = [
        ...
        f"http://0.0.0.0:8080/spotrix/slice/{alert.slice_id}/",
    ]
