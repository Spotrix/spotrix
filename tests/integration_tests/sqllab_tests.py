...
from spotrix import db, security_manager
from spotrix.connectors.sqla.models import SqlaTable
from spotrix.db_engine_specs import BaseEngineSpec
from spotrix.db_engine_specs.hive import HiveEngineSpec
from spotrix.db_engine_specs.presto import PrestoEngineSpec
from spotrix.errors import ErrorLevel, SpotrixError, SpotrixErrorType
from spotrix.exceptions import SpotrixErrorException
from spotrix.models.core import Database
from spotrix.models.sql_lab import LimitingFactor, Query, SavedQuery
from spotrix.result_set import SpotrixResultSet
from spotrix.sql_lab import (
    cancel_query,
...
from spotrix.sql_parse import CtasMethod
from spotrix.utils.core import (
    backend,
    datetime_to_epoch,
...
class TestSqlLab(SupersetTestCase):
    ...
    def test_sql_json(self):
        ...
        if backend() == "presto":
            assert (
                ...
                == SpotrixErrorType.TABLE_DOES_NOT_EXIST_ERROR
            )
            ...
            "errors": [
                {
                    ...
                    "error_type": SpotrixErrorType.DML_NOT_ALLOWED_ERROR,
    ...
    def test_queries_endpoint(self):
        ...
        resp = self.client.get("/spotrix/queries/0")
        # Redirects to the login page
        self.assertEqual(401, resp.status_code)

        # Admin sees queries
        self.login("admin")
        data = self.get_json_resp("/spotrix/queries/0")
        self.assertEqual(2, len(data))
        data = self.get_json_resp("/spotrix/queries/0.0")
        self.assertEqual(2, len(data))
        ...
        data = self.get_json_resp(
            "/spotrix/queries/{}".format(float(datetime_to_epoch(now)) - 1000)
        )
    ...
    def test_search_query_on_db_id(self):
        ...
        data = self.get_json_resp(
            f"/spotrix/search_queries?database_id={examples_dbid}"
        )
        self.assertEqual(3, len(data))
        db_ids = [k["dbId"] for k in data]
        self.assertEqual([examples_dbid for i in range(3)], db_ids)

        resp = self.get_resp("/spotrix/search_queries?database_id=-1")
    ...
    def test_search_query_on_user(self):
        ...
        data = self.get_json_resp("/spotrix/search_queries?user_id={}".format(user_id))
        self.assertEqual(2, len(data))
        user_ids = {k["userId"] for k in data}
        self.assertEqual(set([user_id]), user_ids)

        user_id = security_manager.find_user("gamma_sqllab").id
        resp = self.get_resp("/spotrix/search_queries?user_id={}".format(user_id))
        data = json.loads(resp)
        self.assertEqual(1, len(data))
    ...
    def test_search_query_on_status(self):
        ...
        resp = self.get_resp("/spotrix/search_queries?status=success")
        data = json.loads(resp)
        self.assertEqual(2, len(data))
        states = [k["state"] for k in data]
        self.assertEqual(["success", "success"], states)

        resp = self.get_resp("/spotrix/search_queries?status=failed")
        data = json.loads(resp)
        self.assertEqual(1, len(data))
    ...
    def test_search_query_on_text(self):
        self.run_some_queries()
        self.login("admin")
        url = "/spotrix/search_queries?search_text=birth"
    ...
    def test_search_query_on_time(self):
        ...
        resp = self.get_resp("/spotrix/search_queries?" + "&".join(params))
        data = json.loads(resp)
        self.assertEqual(2, len(data))
    ...
    def test_search_query_only_owned(self) -> None:
        ...
        user_id = security_manager.find_user("gamma_sqllab").id
        data = self.get_json_resp("/spotrix/search_queries")
    ...
    def test_ps_conversion_no_dict(self):
        ...
        data = [["a", 4, 4.0]]
        results = SpotrixResultSet(data, cols, BaseEngineSpec)
    ...
    def test_sqllab_viz(self):
        ...
        resp = self.get_json_resp("/spotrix/sqllab_viz/", data=data)
    ...
    def test_sqllab_table_viz(self):
        ...
        data = {"data": json.dumps(payload)}
        resp = self.get_json_resp("/spotrix/get_or_create_table/", data=data)
        self.assertIn("table_id", resp)
    ...
    def test_execute_sql_statements_no_results_backend(
        ...
        with pytest.raises(SpotrixErrorException) as excinfo:
            execute_sql_statements(
                query_id=1,
                rendered_query=sql,
                return_results=True,
        ...
        assert excinfo.value.error == SpotrixError(
    ...
    def test_execute_sql_statements_ctas(
        ...
        assert excinfo.value.error == SpotrixError(
            ...
            error_type=SpotrixErrorType.INVALID_CTAS_QUERY_ERROR,
            level=ErrorLevel.ERROR,
            extra={
                "issue_codes": [
                    {
        ...
        assert excinfo.value.error == SpotrixError(
            ...
            error_type=SpotrixErrorType.INVALID_CVAS_QUERY_ERROR,
            level=ErrorLevel.ERROR,
            extra={
                "issue_codes": [
                    {
            ...
            "errors": [
                {
                    ...
                    "error_type": SpotrixErrorType.SQLLAB_TIMEOUT_ERROR,
                    "level": ErrorLevel.ERROR,
                    "extra": {
                        "issue_codes": [
                            {
