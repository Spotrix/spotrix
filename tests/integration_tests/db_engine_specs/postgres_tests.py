...
from spotrix.db_engine_specs import get_engine_specs
...
from spotrix.utils.core import GenericDataType
...
class TestPostgresDbEngineSpec(TestDbEngineSpec):
    ...
    def test_extract_errors(self):
        ...
        assert result == [
            SpotrixError(
                error_type=SpotrixErrorType.CONNECTION_INVALID_USERNAME_ERROR,
...
psql: error: could not connect to server: Operation timed out
        ...
        assert result == [
            SpotrixError(
                error_type=SpotrixErrorType.CONNECTION_HOST_DOWN_ERROR,
                message=(
...
psql: error: could not connect to server: Operation timed out
        ...
        assert result == [
            SpotrixError(
                error_type=SpotrixErrorType.CONNECTION_INVALID_PASSWORD_ERROR,
        ...
        assert result == [
            SpotrixError(
                ...
                error_type=SpotrixErrorType.CONNECTION_UNKNOWN_DATABASE_ERROR,
                level=ErrorLevel.ERROR,
                extra={
                    "engine_name": "PostgreSQL",
                    "issue_codes": [
        ...
        assert result == [
            SpotrixError(
                ...
                error_type=SpotrixErrorType.CONNECTION_ACCESS_DENIED_ERROR,
                level=ErrorLevel.ERROR,
                extra={
                    "invalid": ["password"],
                    "engine_name": "PostgreSQL",
        ...
        assert result == [
            SpotrixError(
                ...
                error_type=SpotrixErrorType.SYNTAX_ERROR,
                level=ErrorLevel.ERROR,
                extra={
                    "engine_name": "PostgreSQL",
                    "issue_codes": [
