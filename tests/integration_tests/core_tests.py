...
from spotrix.models.cache import CacheKey
from spotrix.utils.core import get_example_database
from tests.integration_tests.conftest import with_feature_flags
from tests.integration_tests.fixtures.energy_dashboard import (
    load_energy_table_with_slice,
)
from tests.integration_tests.test_app import app
import spotrix.views.utils
from spotrix import (
    dataframe,
    db,
    security_manager,
...
from spotrix.db_engine_specs.base import BaseEngineSpec
from spotrix.db_engine_specs.mssql import MssqlEngineSpec
from spotrix.exceptions import SpotrixException
from spotrix.extensions import async_query_manager
from spotrix.models import core as models
from spotrix.models.annotations import Annotation, AnnotationLayer
...
from spotrix.models.sql_lab import Query
from spotrix.result_set import SpotrixResultSet
from spotrix.utils import core as utils
from spotrix.views import core as views
from spotrix.views.database.views import DatabaseView
...
class TestCore(SupersetTestCase):
    ...
    def test_dashboard_endpoint(self):
        self.login()
        resp = self.client.get("/spotrix/dashboard/-1/")
    ...
    def test_slice_endpoint(self):
        ...
        resp = self.get_resp("/spotrix/slice/{}/".format(slc.id))
        assert "Original value" in resp
        assert "List Roles" in resp

        # Testing overrides
        resp = self.get_resp("/spotrix/slice/{}/?standalone=true".format(slc.id))
        assert '<div class="navbar' not in resp

        resp = self.client.get("/spotrix/slice/-1/")
        assert resp.status_code == 404
    ...
    def test_get_superset_tables_not_allowed(self):
        ...
        uri = f"spotrix/tables/{example_db.id}/{schema_name}/undefined/"
    ...
    def test_get_superset_tables_substr(self):
        ...
        self.login(username="admin")
        schema_name = self.default_schema_backend_map[example_db.backend]
        uri = f"spotrix/tables/{example_db.id}/{schema_name}/ab_role/"
        rv = self.client.get(uri)
    ...
    def test_get_superset_tables_not_found(self):
        self.login(username="admin")
        uri = f"spotrix/tables/invalid/public/undefined/"
        rv = self.client.get(uri)
        self.assertEqual(rv.status_code, 404)
    ...
    def test_annotation_json_endpoint(self):
        ...
        response = self.get_resp(
            f"/spotrix/annotation_json/{layer.id}?form_data="
    ...
    def test_save_slice(self):
        ...
        url = (
            "/spotrix/explore/table/{}/?slice_name={}&"
    ...
    def test_filter_endpoint(self):
        ...
        url = (
            "/spotrix/filter/table/{}/target/?viz_type=sankey&groupby=source"
    ...
    def test_tablemodelview_list(self):
        ...
        assert "/spotrix/explore/table/{}".format(table.id) in resp
    ...
    def test_get_user_slices_for_owners(self):
        ...
        url = f"/spotrix/user_slices/{user.id}/"
        resp = self.client.get(url)
    ...
    def test_testconn(self, username="admin"):
        ...
        response = self.client.post(
            "/spotrix/testconn", data=data, content_type="application/json"
        )
    ...
    def test_warm_up_cache(self):
        self.login()
        slc = self.get_slice("Girls", db.session)
        data = self.get_json_resp("/spotrix/warm_up_cache?slice_id={}".format(slc.id))
        self.assertEqual(
            data, [{"slice_id": slc.id, "viz_error": None, "viz_status": "success"}]
        )

        data = self.get_json_resp(
            "/spotrix/warm_up_cache?table_name=energy_usage&db_name=main"
        )
        assert len(data) > 0
        ...
        assert self.get_json_resp(
            f"/spotrix/warm_up_cache?dashboard_id={dashboard.id}&slice_id={slc.id}"
        ) == [{"slice_id": slc.id, "viz_error": None, "viz_status": "success"}]

        assert self.get_json_resp(
            f"/spotrix/warm_up_cache?dashboard_id={dashboard.id}&slice_id={slc.id}&extra_filters="
    ...
    def test_cache_logging(self):
        ...
        self.get_json_resp("/spotrix/warm_up_cache?slice_id={}".format(girls_slice.id))
    ...
    def test_csv_endpoint(self):
        ...
        resp = self.get_resp("/spotrix/csv/{}".format(client_id))
    ...
    def test_extra_table_metadata(self):
        ...
        self.get_json_resp(
            f"/spotrix/extra_table_metadata/{example_db.id}/birth_names/{schema}/"
        )
    ...
    def test_custom_templated_sql_json(self, sql_lab_mock, mock_dt) -> None:
        ...
        self.get_json_resp(
            "/spotrix/sql_json/", raise_on_error=False, json_=json_payload
        )
    ...
    def test_fetch_datasource_metadata(self):
        self.login(username="admin")
        url = "/spotrix/fetch_datasource_metadata?" "datasourceKey=1__table"
    ...
    def test_user_profile(self, username="admin"):
        ...
        # Setting some faves
        url = f"/spotrix/favstar/Slice/{slc.id}/select/"
        resp = self.get_json_resp(url)
        self.assertEqual(resp["count"], 1)

        dash = db.session.query(Dashboard).filter_by(slug="births").first()
        url = f"/spotrix/favstar/Dashboard/{dash.id}/select/"
        resp = self.get_json_resp(url)
        self.assertEqual(resp["count"], 1)

        userid = security_manager.find_user("admin").id
        resp = self.get_resp(f"/spotrix/profile/{username}/")
        self.assertIn('"app"', resp)
        data = self.get_json_resp(f"/spotrix/recent_activity/{userid}/")
        self.assertNotIn("message", data)
        data = self.get_json_resp(f"/spotrix/created_slices/{userid}/")
        self.assertNotIn("message", data)
        data = self.get_json_resp(f"/spotrix/created_dashboards/{userid}/")
        self.assertNotIn("message", data)
        data = self.get_json_resp(f"/spotrix/fave_slices/{userid}/")
        self.assertNotIn("message", data)
        data = self.get_json_resp(f"/spotrix/fave_dashboards/{userid}/")
        self.assertNotIn("message", data)
        data = self.get_json_resp(f"/spotrix/user_slices/{userid}/")
        self.assertNotIn("message", data)
        data = self.get_json_resp(f"/spotrix/fave_dashboards_by_username/{username}/")
        self.assertNotIn("message", data)
    ...
    def test_dataframe_timezone(self):
        ...
        results = SpotrixResultSet(list(data), [["data"]], BaseEngineSpec)
    ...
    def test_slice_payload_no_datasource(self):
        self.login(username="admin")
        data = self.get_json_resp("/spotrix/explore_json/", raise_on_error=False)
    ...
    def test_explore_json(self):
        ...
        self.login(username="admin")
        rv = self.client.post(
            "/spotrix/explore_json/", data={"form_data": json.dumps(form_data)},
        )
    ...
    def test_explore_json_async_results_format(self):
        ...
        self.login(username="admin")
        rv = self.client.post(
            "/spotrix/explore_json/?results=true",
            data={"form_data": json.dumps(form_data)},
        )
        self.assertEqual(rv.status_code, 200)
    ...
    def test_explore_json_data(self, mock_force_cached, mock_cache):
        ...
        self.login(username="admin")
        rv = self.client.get("/spotrix/explore_json/data/valid-cache-key")
        data = json.loads(rv.data.decode("utf-8"))
    ...
    def test_explore_json_data_invalid_cache_key(self):
        ...
        rv = self.client.get(f"/spotrix/explore_json/data/{cache_key}")
        data = json.loads(rv.data.decode("utf-8"))
    ...
    @mock.patch(
        "superset.security.SpotrixSecurityManager.get_schemas_accessible_by_user"
    )
    @mock.patch("superset.security.SpotrixSecurityManager.can_access_database")
    @mock.patch("superset.security.SpotrixSecurityManager.can_access_all_datasources")
    def test_schemas_access_for_csv_upload_endpoint(
        ...
        data = self.get_json_resp(
            url="/spotrix/schemas_access_for_csv_upload?db_id={db_id}".format(
                db_id=dbobj.id
            )
        )
    ...
    def test_select_star(self):
        ...
        resp = self.get_resp(f"/spotrix/select_star/{examples_db.id}/birth_names")
    ...
    def test_get_select_star_not_allowed(self):
        ...
        self.login(username="gamma")
        example_db = utils.get_example_database()
        resp = self.client.get(f"/spotrix/select_star/{example_db.id}/birth_names")
    ...
    def test_display_limit(self, mock_results_backend):
        ...
        with mock.patch("superset.views.core.db") as mock_superset_db:
            ...
            # get all results
            result_key = json.loads(self.get_resp("/spotrix/results/key/"))
            result_limited = json.loads(self.get_resp("/spotrix/results/key/?rows=1"))
    ...
    def test_results_default_deserialization(self):
        ...
        results = SpotrixResultSet(data, cursor_descr, db_engine_spec)
        query = {
            "database_id": 1,
    ...
    def test_feature_flag_serialization(self):
        ...
        urls = [
            "/spotrix/sqllab",
            "/spotrix/welcome",
            f"/spotrix/dashboard/{dash_id}/",
            "/spotrix/profile/admin/",
            f"/spotrix/explore/table/{tbl_id}",
        ]
        for url in urls:
    ...
    @pytest.mark.usefixtures("load_world_bank_dashboard_with_slices")
    @mock.patch("spotrix.models.core.DB_CONNECTION_MUTATOR")
    def test_explore_injected_exceptions(self, mock_db_connection_mutator):
        ...
        exception = SpotrixException("Error message")
        mock_db_connection_mutator.side_effect = exception
        slice = db.session.query(Slice).first()
        url = f"/spotrix/explore/?form_data=%7B%22slice_id%22%3A%20{slice.id}%7D"
    ...
    def test_dashboard_injected_exceptions(self, mock_db_connection_mutator):
        ...
        url = f"/spotrix/dashboard/{dash.id}/"
