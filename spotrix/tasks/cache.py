...
from spotrix import app, db
from spotrix.extensions import celery_app
from spotrix.models.core import Log
from spotrix.models.dashboard import Dashboard
from spotrix.models.slice import Slice
from spotrix.models.tags import Tag, TaggedObject
from spotrix.utils.date_parser import parse_human_datetime
from spotrix.views.utils import build_extra_filters
...
def get_url(chart: Slice, extra_filters: Optional[Dict[str, Any]] = None) -> str:
    ...
    with app.test_request_context():
        baseurl = (
            "{SPOTRIX_WEBSERVER_PROTOCOL}://"
            "{SPOTRIX_WEBSERVER_ADDRESS}:"
            "{SPOTRIX_WEBSERVER_PORT}".format(**app.config)
        )
...
class Strategy:
    ...
    be fetched from the `/spotrix/warm_up_cache/` endpoint.

    Strategies can be configured in `spotrix/config.py`:
