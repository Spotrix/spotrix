...
migrate to the new scheduler: `spotrix.tasks.scheduler`.
"""
...
from spotrix import app, security_manager, thumbnail_cache
from spotrix.extensions import celery_app, machine_auth_provider_factory
from spotrix.models.alerts import Alert, AlertLog
from spotrix.models.dashboard import Dashboard
from spotrix.models.schedules import (
    EmailDeliveryType,
    get_scheduler_model,
    ScheduleType,
...
from spotrix.tasks.alerts.observer import observe
from spotrix.tasks.alerts.validator import get_validator_function
from spotrix.tasks.slack_util import deliver_slack_msg
from spotrix.utils.celery import session_scope
from spotrix.utils.core import get_email_address_list, send_email_smtp
from spotrix.utils.retries import retry_call
from spotrix.utils.screenshots import ChartScreenshot, WebDriverProxy
...
def _generate_report_content(
    ...
    if delivery_type == EmailDeliveryType.attachment:
        ...
        body = __(
            '<b><a href="%(url)s">Explore in Spotrix</a></b><p></p>',
            name=name,
            url=url,
        )
...
def deliver_dashboard(  # pylint: disable=too-many-locals
    ...
    with session_scope(nullpool=True) as session:
        ...
        dashboard_url_user_friendly = _get_url_path(
            "Spotrix.dashboard", user_friendly=True, dashboard_id_or_slug=dashboard.id
        )
...
def _get_slice_data(
    ...
    slice_url = _get_url_path(
        "Spotrix.explore_json", csv="true", form_data=json.dumps({"slice_id": slc.id})
    )

    # URL to include in the email
    slice_url_user_friendly = _get_url_path(
        "Spotrix.slice", slice_id=slc.id, user_friendly=True
    )
    ...
    if delivery_type == EmailDeliveryType.inline:
        ...
        with app.app_context():
            body = render_template(
                "spotrix/reports/slice_data.html",
                columns=columns,
                rows=rows,
                name=slc.slice_name,
    ...
    slack_message = __(
        """
        *%(slice_name)s*\n
        <%(slice_url_user_friendly)s|Explore in Spotrix>
        """,
...
def _get_slice_screenshot(slice_id: int, session: Session) -> ScreenshotData:
    ...
    chart_url = get_url_path("Spotrix.slice", slice_id=slice_obj.id, standalone="true")
    screenshot = ChartScreenshot(chart_url, slice_obj.digest)
    image_url = _get_url_path(
        "Spotrix.slice", user_friendly=True, slice_id=slice_obj.id,
    )
...
def _get_slice_visualization(
    ...
    slice_url = _get_url_path("Spotrix.slice", slice_id=slc.id)
    slice_url_user_friendly = _get_url_path(
        "Spotrix.slice", slice_id=slc.id, user_friendly=True
    )
...
def deliver_email_alert(alert_content: AlertContent, recipients: str) -> None:
    ...
    subject = f"[Spotrix] Triggered alert: {alert_content.label}"
