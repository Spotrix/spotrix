...
from spotrix.exceptions import SpotrixVizException
...
from spotrix.views.utils import get_datasource_info, get_viz
...
def load_explore_json_into_cache(  # pylint: disable=too-many-locals
    ...
    try:
        ...
        if viz_obj.has_error(payload):
            raise SpotrixVizException(errors=payload["errors"])
        ...
        result_url = f"/spotrix/explore_json/data/{cache_key}"
        async_query_manager.update_job(
            job_metadata, async_query_manager.STATUS_DONE, result_url=result_url,
        )
        ...
        if isinstance(exc, SpotrixVizException):
