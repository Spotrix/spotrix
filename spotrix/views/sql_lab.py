...
from spotrix import db, is_feature_enabled
from spotrix.constants import MODEL_VIEW_RW_METHOD_PERMISSION_MAP, RouteMethod
from spotrix.models.sql_lab import Query, SavedQuery, TableSchema, TabState
from spotrix.x_typing import FlaskResponse
from spotrix.utils import core as utils

from .base import BaseSpotrixView, DeleteMixin, json_success, SupersetModelView
...
class TabStateView(BaseSpotrixView):
    @has_access_api
    @expose("/", methods=["POST"])
...
class TableSchemaView(BaseSpotrixView):
    @has_access_api
    @expose("/", methods=["POST"])
    def post(self) -> FlaskResponse:  # pylint: disable=no-self-use
