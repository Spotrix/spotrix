...
from spotrix import db, event_logger
from spotrix.models import core as models
from spotrix.x_typing import FlaskResponse
from spotrix.views.base import BaseSpotrixView

logger = logging.getLogger(__name__)


class R(BaseSpotrixView):  # pylint: disable=invalid-name
    ...
    def _validate_url(url: Optional[str] = None) -> bool:
        if url and (
            url.startswith("//spotrix/dashboard/")
            or url.startswith("//spotrix/explore/")
        ):
            return True
        return False
