...
from spotrix.charts.dao import ChartDAO
from spotrix.connectors.base.models import BaseDatasource
from spotrix.connectors.connector_registry import ConnectorRegistry
from spotrix.connectors.sqla.models import (
    AnnotationDatasource,
    SqlaTable,
    SqlMetric,
    TableColumn,
)
from spotrix.dashboards.commands.importers.v0 import ImportDashboardsCommand
from spotrix.dashboards.dao import DashboardDAO
from spotrix.databases.dao import DatabaseDAO
from spotrix.databases.filters import DatabaseFilter
from spotrix.datasets.commands.exceptions import DatasetNotFoundError
from spotrix.errors import ErrorLevel, SpotrixError, SpotrixErrorType
from spotrix.exceptions import (
    CacheLoadError,
    CertificateException,
    DatabaseNotFound,
    SerializationError,
    SpotrixCancelQueryException,
    SpotrixErrorException,
    SpotrixCancelErrorsException,
    SpotrixException,
    SpotrixGenericDBErrorException,
    SpotrixGenericErrorException,
    SpotrixSecurityException,
    SpotrixTemplateParamsErrorException,
    SpotrixTimeoutException,
)
from spotrix.extensions import async_query_manager, cache_manager
from spotrix.jinja_context import get_template_processor
from spotrix.models.core import Database, FavStar, Log
from spotrix.models.dashboard import Dashboard
from spotrix.models.datasource_access_request import DatasourceAccessRequest
from spotrix.models.slice import Slice
from spotrix.models.sql_lab import LimitingFactor, Query, TabState
from spotrix.models.user_attributes import UserAttribute
from spotrix.queries.dao import QueryDAO
from spotrix.security.analytics_db_safety import check_sqlalchemy_uri
from spotrix.sql_parse import CtasMethod, ParsedQuery, Table
from spotrix.sql_validators import get_validator_by_name
from spotrix.tasks.async_queries import load_explore_json_into_cache
from spotrix.x_typing import FlaskResponse
from spotrix.utils import core as utils, csv
from spotrix.utils.async_query_manager import AsyncQueryTokenException
from spotrix.utils.cache import etag_cache
from spotrix.utils.core import ReservedUrlParameters
from spotrix.utils.dates import now_as_float
from spotrix.utils.decorators import check_dashboard_access
...
from spotrix.views.utils import (
...
from spotrix.viz import BaseViz
...
class Spotrix(BaseSpotrixView):  # pylint: disable=too-many-public-methods
    """The base views for Spotrix!"""
    ...
    def request_access(self) -> FlaskResponse:
        ...
        if has_access_:
            return redirect("/spotrix/dashboard/{}".format(dashboard_id))
    ...
    def slice(self, slice_id: int) -> FlaskResponse:  # pylint: disable=no-self-use
        ...
        if not slc:
            abort(404)
        endpoint = "/spotrix/explore/?form_data={}".format(
    ...
    def explore_json(
        ...
        if (
            ...
            and not security_manager.can_access("can_csv", "Spotrix")
        ):
    ...
    def import_dashboards(self) -> FlaskResponse:
            ...
            try:
                ...
                flash(
                    _(
                        ...
                        "Please contact your Spotrix administrator"
                    ),
                    "danger",
                )
            if success:
        ...
        return self.render_template(
            "spotrix/import_dashboards.html", databases=databases
        )
    ...
    def explore(
        ...
        slice_download_perm = security_manager.can_access("can_csv", "Spotrix")
        ...
        except (SpotrixException, SQLAlchemyError):
    ...
    def filter(  # pylint: disable=no-self-use
        ...
        :raises SpotrixSecurityException: If the user cannot access the resource
        """
    ...
    def testconn(  # pylint: disable=too-many-return-statements,no-self-use
        ...
        except SpotrixSecurityException as ex:
    ...
    def csrf_token(self) -> FlaskResponse:
        ...
        return Response(
            self.render_template("spotrix/csrf_token.json"), mimetype="text/json"
        )
    ...
    def available_domains(self) -> FlaskResponse:  # pylint: disable=no-self-use
        """
        Returns the list of available Spotrix Webserver domains (if any)
        defined in config. This enables charts embedded in other apps to
        leverage domain sharding if appropriately configured.
        """
        return Response(
            json.dumps(conf.get("SPOTRIX_WEBSERVER_DOMAINS")), mimetype="text/json"
        )
    ...
    def fave_dashboards(  # pylint: disable=no-self-use
        ...
        for o in qry.all():
            ...
            if o.Dashboard.created_by:
                ...
                dash["creator_url"] = "/spotrix/profile/{}/".format(user.username)
            payload.append(dash)
        return json_success(json.dumps(payload, default=utils.json_int_dttm_ser))
        ...
        if config["ENABLE_ACCESS_REQUEST"]:
            for datasource in dashboard.datasources:
                ...
                if datasource and not security_manager.can_access_datasource(
                    ...
                    return redirect(
                        f"/spotrix/request_access/?dashboard_id={dashboard.id}"
                    )

        dash_edit_perm = check_ownership(
            dashboard, raise_if_false=False
        ) and security_manager.can_access("can_save_dash", "Spotrix")
        edit_mode = (
    ...
    def sqllab_viz(self) -> FlaskResponse:  # pylint: disable=no-self-use
        ...
        try:
            ...
            raise SpotrixGenericErrorException(
                __(
        ...
        if not database:
            raise SpotrixErrorException(
                SpotrixError(
                    message=__("The database was not found."),
                    error_type=SpotrixErrorType.DATABASE_NOT_FOUND_ERROR,
        ...
        if not self.appbuilder.sm.can_access_table(database, Table(table_name, schema)):
            ...
            raise SpotrixErrorException(
                SpotrixError(
                    ...
                    error_type=SpotrixErrorType.QUERY_SECURITY_ACCESS_ERROR,
                    level=ErrorLevel.ERROR,
                ),
                status=403,
            )
    ...
    def estimate_query_cost(  # pylint: disable=no-self-use
        ...
        except SpotrixTimeoutException as ex:
            logger.exception(ex)
    ...
    def theme(self) -> FlaskResponse:
        return self.render_template("spotrix/theme.html")
        ...
        if not results_backend:
            raise SpotrixErrorException(
                SpotrixError(
                    ...
                    error_type=SpotrixErrorType.RESULTS_BACKEND_NOT_CONFIGURED_ERROR,
                    level=ErrorLevel.ERROR,
                )
            )
        ...
        if not blob:
            raise SpotrixErrorException(
                SpotrixError(
                    ...
                    error_type=SpotrixErrorType.RESULTS_BACKEND_ERROR,
                    level=ErrorLevel.ERROR,
                ),
                status=410,
            )
    ...
    def stop_query(self) -> FlaskResponse:
        ...
        if not sql_lab.cancel_query(query, g.user.username if g.user else None):
            raise SpotrixCancelQueryException("Could not cancel query")
    ...
    def _sql_json_async(  # pylint: disable=too-many-arguments
        ...
        try:
            ...
            error = SpotrixError(
                message=message,
                error_type=SpotrixErrorType.ASYNC_WORKERS_ERROR,
                level=ErrorLevel.ERROR,
            )
            ...
            raise SpotrixErrorException(error)
    ...
    def _sql_json_sync(
        ...
        :return: A Flask Response
        :raises: SpotrixTimeoutException
        """
        try:
            ...
            raise SpotrixGenericDBErrorException(utils.error_msg_from_exception(ex))

        if data.get("status") == QueryStatus.FAILED:
            # new error payload with rich context
            if data["errors"]:
                raise SpotrixCancelErrorsException(
                    [SpotrixError(**params) for params in data["errors"]]
                )
            # old string-only error message
            raise SpotrixGenericDBErrorException(data["error"])
    ...
    def sql_json_exec(  # pylint: disable=too-many-statements,too-many-locals
        ...
        try:
            ...
            session.commit()
            raise SpotrixErrorException(ex.error, status=403) from ex
        ...
        try:
            ...
            query.status = QueryStatus.FAILED
            session.commit()
            raise SpotrixTemplateParamsErrorException(
                message=__(
                ...
                error=SpotrixErrorType.INVALID_TEMPLATE_PARAMS_ERROR,
            ) from ex
        ...
        if is_feature_enabled("ENABLE_TEMPLATE_PROCESSING"):
            ...
            if undefined_parameters:
                ...
                raise SpotrixTemplateParamsErrorException(
                    ...
                    + " "
                    + PARAMETER_MISSING_ERR,
                    error=SpotrixErrorType.MISSING_TEMPLATE_PARAMS_ERROR,
                    extra={
    ...
    def csv(  # pylint: disable=no-self-use,too-many-locals
        ...
        logger.debug(
            "CSV exported: %s", event_rep, extra={"spotrix_event": event_info}
        )
        return response
    ...
    def show_traceback(self) -> FlaskResponse:  # pylint: disable=no-self-use
        return (
            render_template("spotrix/traceback.html", error_msg=get_error_msg()),
            500,
        )
    ...
    def welcome(self) -> FlaskResponse:
        ...
        if not g.user or not g.user.get_id():
            if conf.get("PUBLIC_ROLE_LIKE_GAMMA", False) or conf["PUBLIC_ROLE_LIKE"]:
                return self.render_template("spotrix/public_welcome.html")
    ...
    def sqllab(self) -> FlaskResponse:
        ...
        return self.render_template(
            "spotrix/basic.html", entry="sqllab", bootstrap_data=bootstrap_data
        )
    ...
    def sqllab_history(self) -> FlaskResponse:
        if not is_feature_enabled("ENABLE_REACT_CRUD_VIEWS"):
            return redirect("/spotrix/sqllab#search", code=307)
    ...
    def schemas_access_for_csv_upload(self) -> FlaskResponse:
        ...
        try:
            ...
            return json_error_response(
                ...
                "Please contact your Spotrix Admin!"
            )
