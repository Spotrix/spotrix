...
from spotrix.commands.exceptions import CommandException, CommandInvalidError
from spotrix.connectors.sqla import models
from spotrix.datasets.commands.exceptions import get_dataset_exist_error_msg
...
from spotrix.models.helpers import ImportExportMixin
from spotrix.translations.utils import get_language_pack
...
if TYPE_CHECKING:
    from spotrix.connectors.druid.views import (  # pylint: disable=unused-import
        DruidClusterModelView,
    )

FRONTEND_CONF_KEYS = (
    "SPOTRIX_WEBSERVER_TIMEOUT",
    "SPOTRIX_DASHBOARD_POSITION_DATA_LIMIT",
    "SPOTRIX_DASHBOARD_PERIODICAL_REFRESH_LIMIT",
...
def api(f: Callable[..., FlaskResponse]) -> Callable[..., FlaskResponse]:
    ...
    def wraps(self: "BaseSpotrixView", *args: Any, **kwargs: Any) -> FlaskResponse:
        try:
...
def handle_api_exception(
    ...
    """
    A decorator to catch spotrix exceptions. Use it after the @api decorator above
    so spotrix exception handler is triggered before the handler for generic
    exceptions.
    """
    ...
    def wraps(self: "BaseSpotrixView", *args: Any, **kwargs: Any) -> FlaskResponse:
        ...
        except SpotrixCancelErrorsException as ex:
            logger.warning(ex, exc_info=True)
            return json_errors_response(errors=ex.errors, status=ex.status)
        except SpotrixErrorException as ex:
            logger.warning(ex)
...
class BaseSpotrixView(BaseView):
    @staticmethod
    def json_response(
...
def menu_data() -> Dict[str, Any]:
    ...
    return {
        ...
        "navbar_right": {
            ...
            "show_watermark": ("spotrix-logo-horiz" not in appbuilder.app_icon),
            ...
            else f"/spotrix/profile/{g.user.username}",
...
# SpotrixErrorException or SpotrixCancelErrorsException
@spotrix_app.errorhandler(SpotrixErrorException)
def show_superset_error(ex: SpotrixErrorException) -> FlaskResponse:
    logger.warning(ex)
    return json_errors_response(errors=[ex.error], status=ex.status)


@spotrix_app.errorhandler(SpotrixCancelErrorsException)
def show_superset_errors(ex: SpotrixCancelErrorsException) -> FlaskResponse:
    logger.warning(ex)
    return json_errors_response(errors=ex.errors, status=ex.status)


# Redirect to login if the CSRF token is expired
@spotrix_app.errorhandler(CSRFError)
...
@spotrix_app.errorhandler(HTTPException)
def show_http_exception(ex: HTTPException) -> FlaskResponse:
    ...
    if (
        ...
        path = resource_filename("spotrix", f"static/assets/{ex.code}.html")
...
# CommandException it should be fixed to map it to SpotrixErrorException
# or SpotrixCancelErrorsException, with a specific status code and error type
@spotrix_app.errorhandler(CommandException)
def show_command_errors(ex: CommandException) -> FlaskResponse:
    ...
    return json_errors_response(
        errors=[
            SpotrixError(
                message=ex.message,
                error_type=SpotrixErrorType.GENERIC_COMMAND_ERROR,
...
@spotrix_app.errorhandler(Exception)
...
class SupersetListWidget(ListWidget):  # pylint: disable=too-few-public-methods
    template = "spotrix/fab_overrides/list.html"
...
class ListWidgetWithCheckboxes(ListWidget):  # pylint: disable=too-few-public-methods
    ...
    template = "spotrix/fab_overrides/list_with_checkboxes.html"
...
def check_ownership(obj: Any, raise_if_false: bool = True) -> bool:
    ...
    security_exception = SpotrixSecurityException(
        SpotrixError(
            error_type=SpotrixErrorType.MISSING_OWNERSHIP_ERROR,
