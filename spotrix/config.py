...
All configuration in this file can be overridden by providing a spotrix_config
in your PYTHONPATH as there is a ``from spotrix_config import *``
...
from spotrix.jinja_context import (  # pylint: disable=unused-import
    BaseTemplateProcessor,
)
from spotrix.stats_logger import DummyStatsLogger
from spotrix.x_typing import CacheConfig
from spotrix.utils.core import is_test, parse_boolean_string
from spotrix.utils.encrypt import SQLAlchemyUtilsAdapter
from spotrix.utils.log import DBEventLogger
from spotrix.utils.logging_configurator import DefaultLoggingConfigurator
...
if TYPE_CHECKING:
    ...
    from spotrix.connectors.sqla.models import (  # pylint: disable=unused-import
        SqlaTable,
    )
    from spotrix.models.core import Database  # pylint: disable=unused-import

# Realtime stats logger, a StatsD implementation exists
STATS_LOGGER = DummyStatsLogger()
EVENT_LOGGER = DBEventLogger()

SPOTRIX_LOG_VIEW = True

BASE_DIR = os.path.abspath(os.path.dirname(__file__))
if "SPOTRIX_HOME" in os.environ:
    DATA_DIR = os.environ["SPOTRIX_HOME"]
else:
    DATA_DIR = os.path.join(os.path.expanduser("~"), ".spotrix")
...
WTF_CSRF_EXEMPT_LIST = ["spotrix.views.core.log", "spotrix.charts.api.data"]
...
APP_ICON = "/static/assets/images/spotrix-logo.svg"
APP_ICON_WIDTH = 126

# Specify where clicking the logo would take the user
# e.g. setting it to '/' would take the user to '/spotrix/welcome/'
...
# Enables SWAGGER UI for spotrix openapi spec
...
BABEL_DEFAULT_FOLDER = "spotrix/translations"
...
# overwritten by those specified under FEATURE_FLAGS in spotrix_config.py
# For example, DEFAULT_FEATURE_FLAGS = { 'FOO': True, 'BAR': False } here
# and FEATURE_FLAGS = { 'BAR': True, 'BAZ': True } in spotrix_config.py
# will result in combined feature flags of { 'FOO': True, 'BAR': True, 'BAZ': True }
DEFAULT_FEATURE_FLAGS: Dict[str, bool] = {
    # allow dashboard to use sub-domains to send chart request
    # you also need ENABLE_CORS and
    # SPOTRIX_WEBSERVER_DOMAINS for list of domains
    ...
    # encrypted before being saved into spotrix metastore.
...
# Feature flags may also be set via 'SPOTRIX_FEATURE_' prefixed environment vars.
DEFAULT_FEATURE_FLAGS.update(
    {
        k[len("SPOTRIX_FEATURE_") :]: parse_boolean_string(v)
        for k, v in os.environ.items()
        if re.search(r"^SPOTRIX_FEATURE_\w+", k)
    }
)
...
# THEME_OVERRIDES is used for adding custom theme to spotrix
...
# Default cache for spotrix objects
...
# next available socket. PR #5039 is trying to allow domain sharding for Spotrix,
# and this feature will be enabled by configuration only (by default Spotrix
# doesn't allow cross-domain request).
SPOTRIX_WEBSERVER_DOMAINS = None
...
# time grains in spotrix/db_engine_specs.builtin_time_grains).
...
DEFAULT_MODULE_DS_MAP = OrderedDict(
    [
        ("spotrix.connectors.sqla.models", ["SqlaTable"]),
        ("spotrix.connectors.druid.models", ["DruidDatasource"]),
    ]
)
...
FILENAME = os.path.join(DATA_DIR, "spotrix.log")
ROLLOVER = "midnight"
INTERVAL = 1
BACKUP_COUNT = 30
...
if not MAPBOX_API_KEY:
    MAPBOX_API_KEY = "pk.eyJ1IjoiY2l1cyIsImEiOiJja3VjOTA5Z3oweW40MzBxcHRycTVlbHdlIn" \
                     "0.ebUZCJ2u7f1pwcBC_Ie4jA"
...
class CeleryConfig:  # pylint: disable=too-few-public-methods
    ...
    CELERY_IMPORTS = ("spotrix.sql_lab", "spotrix.tasks")
...
# Additional static HTTP headers to be served by your Spotrix server. Note
...
# Roles that are controlled by the API / Spotrix and should not be changes
# by humans.
ROBOT_PERMISSION_ROLES = ["Public", "Gamma", "Alpha", "Admin", "sql_lab"]

CONFIG_PATH_ENV_VAR = "SPOTRIX_CONFIG_PATH"
...
# Slack API token for the spotrix reports, either string or callable
...
MACHINE_AUTH_PROVIDER_CLASS = "spotrix.utils.machine_auth.MachineAuthProvider"
...
# Send user to a link where they can read more about Spotrix
...
# Some sqlalchemy connection strings can open Spotrix to security risks.
...
# SIP-15 should be enabled for all new Spotrix deployments which ensures that the time
...
# (spotrix.connectors.sqla.models.SqlaTable), we call this hook
...
if CONFIG_PATH_ENV_VAR in os.environ:
    ...
    try:
        ...
        override_conf = imp.load_source("spotrix", cfg_path)
        ...
        raise
elif importlib.util.find_spec("spotrix_config") and not is_test():
    try:
        import spotrix_config  # pylint: disable=import-error
        from spotrix_config import *  # type: ignore # pylint: disable=import-error,wildcard-import,unused-wildcard-import

        print(f"Loaded your LOCAL configuration at [{spotrix_config.__file__}]")
    except Exception:
        logger.exception("Found but failed to import local spotrix_config")
        raise
