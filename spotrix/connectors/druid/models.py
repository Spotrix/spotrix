...
from spotrix import conf, db, security_manager
from spotrix.connectors.base.models import BaseColumn, BaseDatasource, BaseMetric
from spotrix.constants import NULL_STRING
from spotrix.exceptions import SpotrixException
from spotrix.extensions import encrypted_field_factory
from spotrix.models.core import Database
from spotrix.models.helpers import AuditMixinNullable, ImportExportMixin, QueryResult
from spotrix.x_typing import (
    AdhocMetric,
    FilterValues,
    Granularity,
    Metric,
...
from spotrix.utils.date_parser import parse_human_datetime, parse_human_timedelta
from spotrix.utils.memoized import memoized
...
try:
    from spotrix.utils.core import DimSelector, DTTM_ALIAS, FilterOperator, flasher
except ImportError:
    pass
...
class DruidDatasource(Model, BaseDatasource):
    ...
    def datasource_link(self) -> str:
        url = f"/spotrix/explore/{self.type}/{self.id}/"
        name = escape(self.datasource_name)
    ...
    def get_aggregations(
        ...
        :raise SpotrixException: if one or more metric names are not aggregations
        """
    ...
    def run_query(  # druid
        ...
        if is_rowcount:
            raise SpotrixException("is_rowcount is not supported on Druid connector")
        ...
        if row_offset:
            raise SpotrixException("Offset not implemented for Druid connector")
