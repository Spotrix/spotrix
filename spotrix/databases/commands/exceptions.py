...
from spotrix.commands.exceptions import (
    CommandException,
    CommandInvalidError,
    CreateFailedError,
    DeleteFailedError,
...
class DatabaseTestConnectionFailedError(SpotrixCancelErrorsException):
    status = 422
    message = _("Connection failed, please check your connection settings")
...
class DatabaseTestConnectionUnexpectedError(SpotrixCancelErrorsException):
    status = 422
...
class InvalidEngineError(SpotrixErrorException):
    status = 422


class DatabaseOfflineError(SpotrixErrorException):
    status = 422


class InvalidParametersError(SpotrixCancelErrorsException):
    status = 422
