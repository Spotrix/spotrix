...
from spotrix import cache_manager, is_feature_enabled
from spotrix.db_engine_specs.base import BaseEngineSpec
from spotrix.errors import SpotrixErrorType
from spotrix.exceptions import SpotrixTemplateException
from spotrix.models.sql_lab import Query
from spotrix.models.sql_types.presto_sql_types import (
    Array,
    Interval,
    Map,
    Row,
...
from spotrix.result_set import destringify
from spotrix.sql_parse import ParsedQuery
from spotrix.utils import core as utils
from spotrix.utils.core import ColumnSpec, GenericDataType
...
class PrestoEngineSpec(BaseEngineSpec):  # pylint: disable=too-many-public-methods
    ...
    custom_errors: Dict[Pattern[str], Tuple[str, SpotrixErrorType, Dict[str, Any]]] = {
        COLUMN_DOES_NOT_EXIST_REGEX: (
            __(
                'We can\'t seem to resolve the column "%(column_name)s" at '
                "line %(location)s.",
            ),
            SpotrixErrorType.COLUMN_DOES_NOT_EXIST_ERROR,
            {},
        ),
        ...
        SCHEMA_DOES_NOT_EXIST_REGEX: (
            ...
            SpotrixErrorType.SCHEMA_DOES_NOT_EXIST_ERROR,
            {},
        ),
        CONNECTION_ACCESS_DENIED_REGEX: (
            __('Either the username "%(username)s" or the password is incorrect.'),
            SpotrixErrorType.CONNECTION_ACCESS_DENIED_ERROR,
            {},
        ),
        CONNECTION_INVALID_HOSTNAME_REGEX: (
            __('The hostname "%(hostname)s" cannot be resolved.'),
            SpotrixErrorType.CONNECTION_INVALID_HOSTNAME_ERROR,
            {},
        ),
        CONNECTION_HOST_DOWN_REGEX: (
            ...
            SpotrixErrorType.CONNECTION_HOST_DOWN_ERROR,
            {},
        ),
        CONNECTION_PORT_CLOSED_REGEX: (
            __('Port %(port)s on hostname "%(hostname)s" refused the connection.'),
            SpotrixErrorType.CONNECTION_PORT_CLOSED_ERROR,
            {},
        ),
        CONNECTION_UNKNOWN_DATABASE_ERROR: (
            __('Unable to connect to catalog named "%(catalog_name)s".'),
            SpotrixErrorType.CONNECTION_UNKNOWN_DATABASE_ERROR,
            {},
        ),
    }
    ...
    def latest_partition(
        ...
        if not indexes:
            raise SpotrixTemplateException(
    ...
    def latest_sub_partition(
        ...
        for k in kwargs.keys():  # pylint: disable=consider-iterating-dictionary
            if k not in k in part_fields:  # pylint: disable=comparison-with-itself
                ...
                raise SpotrixTemplateException(msg)
